...
#    print vmi['fq_name']
    prefix_len = '32'
    aap_mode='active-active'
    vmi_fq_name = ['default-domain','admin','test-bfd-2-hc-vmi.st%d'%i]
    vmi = vnc_lib.virtual_machine_interface_read(fq_name=vmi_fq_name)
    vmi_prop =  vmi.get_virtual_machine_interface_properties()
    vmi_prop.local_preference = None 
    vmi.set_virtual_machine_interface_properties(vmi_prop)
    aaps = vmi.get_virtual_machine_interface_allowed_address_pairs()
    aaps.allowed_address_pair[0].address_mode = 'active-active'
