...
parent_vmi_1_uuid = 'ee9e9457-fcc1-409a-af38-baa11e0e43d0'
parent_vmi_2_uuid = '51eb5cb8-5518-4e8b-b771-3a5d377ba4c9'
...
client = VncApi(username='admin', password='c0ntrail123', tenant_name='admin', api_server_host='10.87.64.129', api_server_port=8082, auth_host='5.5.5.251')
...
for vlan in range(2, 425):
    ...
    for p_vmi in [p_vmi_1, p_vmi_2]:
        if p_vmi == p_vmi_1:
            vmi_fq_name = ['default-domain','admin','test-bfd-hc-vmi.st%d'%vlan]
        else :
            vmi_fq_name = ['default-domain','admin','test-bfd-2-hc-vmi.st%d'%vlan]
...
#        vmi_obj.add_service_health_check(hc_obj)
        vmi_obj.set_virtual_machine_interface_mac_addresses(p_vmi.virtual_machine_interface_mac_addresses)
